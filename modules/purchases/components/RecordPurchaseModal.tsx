...
export function RecordPurchaseModal({ products, units }: RecordPurchaseModalProps) {
  ...
  const handleQuickAddSave = async () => {
    if (!quickAddName.trim() || !quickAddItemId) return

    setIsPending(true)
    try {
      const res = await createNewMenuItem(quickAddName)

      if (res.success && res.product) {
        toast.success(`'${quickAddName}' sikeresen létrehozva!`)
      ...
      {isQuickAddOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h3 className="text-xl font-semibold mb-4">Új termék</h3>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                disabled={isPending || !quickAddName.trim()}
