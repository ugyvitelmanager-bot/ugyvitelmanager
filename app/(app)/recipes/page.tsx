...
import { BookOpen, Utensils, Info, Printer, Search, AlertTriangle } from 'lucide-react'
...
export default async function RecipesPage({ searchParams }: PageProps) {
  try {
    ...
    // Segédfüggvény a joined adatok biztonságos eléréséhez
    const getSafeJoinedData = (data: any) => {
      if (!data) return null
      return Array.isArray(data) ? data[0] : data
    }

    // Kategória szűrés (kliens-oldali filter a server componentben, mert a product-on keresztül van)
    if (categoryFilter) {
      recipes = recipes.filter(r => {
        const prod = getSafeJoinedData(r.product)
        ...
        {recipes.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-12">
            {recipes.map((recipe) => {
              ...
              const prod = getSafeJoinedData(recipe.product)

              const totalCost = items.reduce((sum: number, item: any) => {
                const ingredient = getSafeJoinedData(item.ingredient)
                const itemCost = (item.quantity * (ingredient?.purchase_price_net || 0))
                return sum + itemCost
              }, 0) || 0
              
              const saleGross = prod?.default_sale_price_gross || 0
              const profitStr = saleGross > totalCost ? formatCurrency(saleGross - totalCost) : 'Alacsony Árrés'
                  ...
                  <CardContent className="p-0">
                    <Table>
                      ...
                      <TableBody>
                        {items.map((item: any) => {
                          const ingredient = getSafeJoinedData(item.ingredient)
                          const unit = getSafeJoinedData(item.unit)
                    ...
                    <div className="p-4 bg-primary/5 flex justify-between items-center text-xs">
                      <div className="flex items-center gap-2 text-primary font-bold">
                        ...
                        Az árrés: <span className="bg-white px-2 py-0.5 rounded shadow-sm">{profitStr}</span>
                      </div>
    ...
    return (
      <div className="p-8 flex items-center justify-center min-h-[50vh]">
        <div className="bg-red-50 border border-red-200 p-6 rounded-xl max-w-md text-center">
          ...
          <h2 className="text-lg font-bold text-red-800 mb-2">Hiba történt a receptek betöltésekor</h2>
          <p className="text-sm text-red-600 mb-4">{error.message || 'Hiba az adatbázis lekérdezésben.'}</p>
